...
class painter {
    private:
        ...
        void init_textures() { // assumes command list and pipeline state object are created
            ...
            check_output(m_device->CreateCommittedResource(
            ...
            D3D12_SUBRESOURCE_DATA texture_data = {.pData = m_bmp_bits,
                                                   .RowPitch = m_bmp_width * BMP_PX_SIZE,
                                                   .SlicePitch =
                                                       m_bmp_width * m_bmp_height * BMP_PX_SIZE};
            // ... ID3D12GraphicsCommandList::Reset() - to dlatego lista
            // poleceń i obiekt stanu potoku muszą być wcześniej utworzone

            m_commandList[0]->Reset(m_commandAllocator[0].Get(), m_pipelineState.Get());
            ...
            m_commandList[0]->CopyTextureRegion(&Dst, 0, 0, 0, &Src, nullptr);
            ...
            m_commandList[0]->ResourceBarrier(1, &tex_upload_resource_barrier);
            check_output(m_commandList[0]->Close());

            ID3D12CommandList *cmd_list = m_commandList[0].Get();
